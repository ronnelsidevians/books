...
DATA = ROOT / 'data'
COVERS = ROOT / 'covers'
TR = {'а':'a','б':'b','в':'v','г':'h','ґ':'g','д':'d','е':'e','є':'ie','ж':'zh','з':'z','и':'y','і':'i','ї':'i','й':'i','к':'k','л':'l','м':'m','н':'н'.replace('н','n'),'о':'o','п':'p','р':'r','с':'s','т':'t','у':'u','ф':'f','х':'kh','ц':'ts','ч':'ch','ш':'sh','щ':'shch','ь':'','ю':'iu','я':'ia','ы':'y','э':'e','ъ':'','ё':'yo'}
...
def parse(pdf):
    ...
    series = '' if rel.parent.as_posix() == '.' else rel.parent.as_posix()
    order = None
...
def cover(pdf, out, zoom=2.0):
    try:
        ...
        print('Cover fallback:', pdf.name, e)
...
def main():
    ...
    pdfs = sorted([x for x in BOOKS.rglob('*') if x.is_file() and x.suffix.lower() == '.pdf' and not any(part.startswith('.') for part in x.relative_to(BOOKS).parts)], key=lambda x: x.relative_to(BOOKS).as_posix().lower())
    used = set()
    books = []
    by_series = {}
    all_folders = set()
    ...
    for pdf in pdfs:
        ...
        base = slug((series + '-' if series else '') + title)
        ident = base
        i = 2
        while ident in used:
            ident = f'{base}-{i}'
        ...
        rel = 'books/' + qpath(pdf.relative_to(BOOKS).parts)
    ...
    # Include physical folders even if they currently have only icon.* or subfolders.
    ...
    series_list = []
    for s in sorted(all_folders, key=lambda x: (x.count('/'), x.lower())):
        direct = sorted(by_series.get(s, []), key=lambda b: (b.get('order') if b.get('order') is not None else 999999, b['title'].lower()))
        desc = descendant_books(s)
        parent = Path(s).parent.as_posix()
        if parent == '.':
            parent = ''
        title = Path(s).name
        icon = folder_icon(BOOKS / s)
        if icon:
            icon_rel = icon.relative_to(BOOKS)
            cover_url = 'books/' + qpath(icon_rel.parts)
            cover_source = 'icon'
        else:
        ...
        series_list.append({'id': slug(s), 'title': title, 'path': s, 'parent': parent, 'cover': cover_url, 'coverSource': cover_source, 'count': len(desc), 'bookIds': [b['id'] for b in desc], 'directBookIds': [b['id'] for b in direct]})
    ...
    print(f'Folders/series found: {len(series_list)}')
