...
def main():
    ...
    used=set(); books=[]; by_series={}; all_folders=set()

    for pdf in pdfs:
        series, order, title = parse(pdf)
        if series:
            parts = series.split('/')
            for i in range(1, len(parts)+1): all_folders.add('/'.join(parts[:i]))
        base = slug((series+'-' if series else '') + title); ident = base; i = 2
        while ident in used:
            ident = f'{base}-{i}'; i += 1
        used.add(ident)
        cf = COVERS / f'{ident}.jpg'; cover(pdf, cf, a.cover_zoom)
        # ВАЖЛИВО: шлях лишаємо НЕ percent-encoded. app.js сам робить encodeURI().
    ...
    for d in sorted([x for x in BOOKS.rglob('*') if x.is_dir() and not any(part.startswith('.') for part in x.relative_to(BOOKS).parts)], key=lambda x:x.relative_to(BOOKS).as_posix().lower()):
        ...
        if rel and rel != '.':
            all_folders.add(rel)
            parts = rel.split('/')
            for i in range(1, len(parts)): all_folders.add('/'.join(parts[:i]))
    ...
    for s in sorted(all_folders, key=lambda x:(x.count('/'), x.lower())):
        ...
        parent = Path(s).parent.as_posix(); parent = '' if parent == '.' else parent
        title = Path(s).name
        icon = folder_icon(BOOKS / s)
        if icon:
            # ВАЖЛИВО: icon шлях теж НЕ кодуємо, бо browser/app.js сам нормально обробить кирилицю.
